...
import { getFirestore, collection, query, orderBy, getDoc,doc, getDocs } from 'firebase/firestore';
    ...
    return (
        <Layout>
            <div>
                ...
                <div>
                    <button onClick={() => router.push(`/blogs/${userId}/create`)}>Create Blog</button>
                </div>
                    ...
                    <ul>
                        {blogs.map((blog) => (
                            <li key={blog.id}>
                                ...
                                <p>Created at: {new Date(blog.createdAt.seconds * 1000).toLocaleString()}</p>
                            </li>
                        ))}
                    </ul>
                )}
